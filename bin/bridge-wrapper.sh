...
if [ -z "${TELEGRAM_CHAT_ID:-}" ] && [ "${CLAUDE_TELEGRAM_BRIDGE_DISABLE_SOPS:-0}" != "1" ]; then
  if [ -f "$SOPS_AGE_KEY_FILE" ] && [ -f ~/.secrets/openclaw-runtime.env ]; then
    TELEGRAM_CHAT_ID=$(SOPS_AGE_KEY_FILE="$SOPS_AGE_KEY_FILE" sops --config "$SOPS_CFG" \
      -d ~/.secrets/openclaw-runtime.env 2>/dev/null | \
      grep '^CLAUDE_CODE_BRIDGE_TELEGRAM_CHAT_ID=' | cut -d= -f2- | tr -d '\n')
  fi
fi
